...
public class FollowService {
    ...
    public FollowResponseDto makeFollow(
        ...
        // -> 팔로우 삭제
            ...
            followRepository.delete(new Follow(follower, following));
            return FollowResponseDto.of(new Follow(follower,following));
        }
    }

    @Transactional
    public FollowResponseDto approveFollow(String followername, UserDetailsImpl userDetails)throws AccessDeniedException{
        User following = userRepository.findByUsername(userDetails.getUsername()).orElseThrow(
                () ->  new UsernameNotFoundException("인증된 유저가 아닙니다")
        );
        ...
        Follow follow = followRepository.findByFollowingAndFollower(following,follower).orElseThrow(
                ()-> new EntityNotFoundException("팔로우 유저의 팔로우 신청을 확인할 수 없습니다")
        );
        follow.approve();
        return FollowResponseDto.of(new Follow(follower,following));
    }
    ...
    @Transactional
    public void denyFollow(String followername, UserDetailsImpl userDetails) throws AccessDeniedException {
        User following = userRepository.findByUsername(userDetails.getUsername()).orElseThrow(
                () -> new UsernameNotFoundException("인증된 유저가 아닙니다")
        );
