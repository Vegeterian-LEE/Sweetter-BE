...
public class PostService {
    ...
    public UserPageDto getUserPage(Long userId, UserDetailsImpl userDetails) {
        ...
        for (int i = 0; i< retweetList.size(); i++){
            boolean retweetCheck = !retweetRepository.findAllByUserIdAndPostId(user.getId(),retweetList.get(i).getPost().getId()).isEmpty();
            tweetList.add(new PostResponseDto(retweetList.get(i).getPost(), false, retweetCheck));
        }
        for (int i = 0; i < postList.size(); i++) {
            boolean retweetCheck = !retweetRepository.findAllByUserIdAndPostId(user.getId(),retweetList.get(i).getPost().getId()).isEmpty();
            tweetList.add(new PostResponseDto(postList.get(i),false, retweetCheck));
        }
        // 중복 삭제처리
        Collections.sort(tweetList, ((o1, o2) -> (int)(o1.getId() - o2.getId())));
        tweetList = DeduplicationUtils.deduplication(tweetList, PostResponseDto::getId);
